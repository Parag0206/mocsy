...
if (lat .lt. -86.0 .or. lat .gt. 90.0) then
   gsw_saar = gsw_error_code(2,func_name)
   return
end if

long_tmp = long
if (long_tmp.lt.0) long_tmp = long_tmp + 360.0

dlong = longs_ref(2) - longs_ref(1)
dlat = lats_ref(2) - lats_ref(1)

indx0 = floor(1.0 + (nx-1.0)*(long_tmp-longs_ref(1)) / &
		(longs_ref(nx)-longs_ref(1)))
if (indx0.eq.nx) indx0 = nx-1

indy0 = floor(1.0 + (ny-1.0)*(lat-lats_ref(1))/(lats_ref(ny)-lats_ref(1)))
...
if (  longs_pan(1).le.long_tmp .and. long_tmp.le.longs_pan(npan)-0.001 .and.&
...
else if (abs(sum(saar)).ge.1e10) then
  saar_old = saar
  call gsw_add_mean(saar_old,saar)
end if

sa_upper = (1.0-s1)*(saar(1) + r1*(saar(2)-saar(1))) + s1*(saar(4) + &
		r1*(saar(3)-saar(4)))
...
sa_lower = (1.0-s1)*(saar(1) + r1*(saar(2)-saar(1))) + s1*(saar(4) + &
		r1*(saar(3)-saar(4)))
if (abs(sa_lower).ge.1e10) sa_lower = sa_upper
gsw_saar = sa_upper + t1*(sa_lower-sa_upper)

if (abs(gsw_saar).ge.1e10) gsw_saar = gsw_error_code(3,func_name)
